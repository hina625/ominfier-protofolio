...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { name: "Contact", href: "/portfolio#contact", icon: <MessageCircle className="w-4 h-4" /> },
  ];
          ...
          <div className="py-4 space-y-4 border-t border-white/10">
            {navItems.map((item, index) => (
              <motion.a
                ...
                className="flex items-center gap-3 text-gray-300 hover:text-white transition-colors duration-300 py-2 px-4 rounded-lg hover:bg-white/5"
              >
                <span>{item.icon}</span>
                <span className="font-medium">{item.name}</span>
              </motion.a>
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => setIsMobileMenuOpen(false)}
              className="w-full flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold rounded-lg shadow-lg"
            >
              <Settings className="w-4 h-4" />
                  Login
